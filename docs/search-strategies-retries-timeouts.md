...
| Cards Central | JSON API (`/api/lgs/search?q=…`) | 5s | Direct → **residential** → dedicated → dynamic (`PreferResidentialProxy`; browser JSON headers + `SkipWebBotAuth`) | Transport fallback only |
