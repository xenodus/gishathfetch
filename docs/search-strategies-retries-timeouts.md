...
| CK pricelist direct attempt timeout | 3m (when proxy fallback configured) | `ckPricelistDirectAttemptTimeout` in `api/gateway/cardkingdom/pricelist_fetch.go` | Caps the first direct download attempt so a slow direct try leaves enough of the shared 14m fetch deadline for the residential proxy retry. |
