...
| CK pricelist transport | direct → residential → dedicated | `downloadCKPricelist` in `api/gateway/cardkingdom/pricelist_fetch.go` | Tries direct egress first, then `RESIDENTIAL_PROXY_1`, then a dedicated proxy when configured. Each step is a single transport attempt. |
