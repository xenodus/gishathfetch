...
| 5 Mana Shopify suggest API | Direct → one random dedicated proxy → dynamic fallback | `api/gateway/fivemana/search.go` | Tries `GET /search/suggest.json` first (JSON, in-stock products only). Falls back to the full `/search` HTML scrape when suggest errors or returns no cards. Reuses the same outbound proxy chain and SGD currency cookies as other Shopify storefront scrapers. |
