...
| 5 Mana Shopify suggest API | Direct → one random dedicated proxy → dynamic fallback | `api/gateway/fivemana/search.go` | Uses Shopify `GET /search/suggest.json` (JSON, in-stock products only) instead of the full themed search HTML page. Reuses the same outbound proxy chain and SGD currency cookies as other Shopify storefront scrapers. |
