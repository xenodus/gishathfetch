...
| “Retries” | N/A (sequential fallbacks) | `runFallbackAttempts` in `storefront_fallback.go` | Stops on the first attempt that returns **cards**. An error-free attempt with **zero cards** is inconclusive and the next strategy runs; only the final attempt may return empty. When **decklist leads** and the first decklist attempt is empty, remaining decklist attempts are skipped and scrap strategies run next. Returns the last annotated error if all attempts fail. This is **not** exponential backoff retry of a single request. |
