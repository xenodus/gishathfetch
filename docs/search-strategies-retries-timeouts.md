...
| 5 Mana Shopify search | Residential → dedicated → dynamic | `api/gateway/fivemana/search.go` | Skips direct requests. Tries `RESIDENTIAL_PROXY_1` first, then one dedicated proxy, then dynamic fallback. |
